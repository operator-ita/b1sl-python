...
A small number of SAP endpoints expose **bounded functions** — operations scoped to a specific entity key (e.g. `SQLQueries('sql04')/List`). These require logic beyond generic CRUD. Every resource exposes a public `action(key, name, ...)` for bound actions/functions and `function(name, params)` for unkeyed ones — call them directly (`invoices.action(123, "Cancel")`) or wrap them in a hand-written resource class that extends `GenericResource` or `AsyncGenericResource`. `SQLQueriesResource` (`resources/sql_queries.py`) is the canonical reference: it adds `run()` and `run_stream()` on top of inherited CRUD, reusing the same ETag, dry-run, and 401-retry infrastructure. See [15-sql-queries.md](./15-sql-queries.md).

For **unbound service operations** at the service root (e.g. `SBOBobService_SetCurrencyRate`), use `client.call_service_method(name, payload)` — the low-level escape hatch for the ~1000 `*Service_*` operations without an Elite alias. Both "actions" and "functions" of this kind are POSTs with a JSON body in the Service Layer, so a single entry point covers both. No ETag concurrency applies; dry-run and exception mapping work as usual.
