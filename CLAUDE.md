...
Non-JSON bodies also have public primitives — `client.post_multipart(endpoint, files)` (multipart upload, honours dry-run) and `client.get_binary(endpoint, params)` (raw bytes, no JSON/text decoding). `client.attachments` is the typed wrapper over both for `Attachments2` (`.upload()` / `.download()`); it is a specialized resource, **not** an Elite alias — no ETag concurrency, and SAP never implemented `DELETE` for it. The working upload body is files-only with the fixed field name `files`, not the two-part shape SAP's manual documents. See `docs/19-attachments.md` and `docs/18-sap-version-quirks.md` Q4.
