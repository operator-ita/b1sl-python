...
try:
    ...
    from mapper import map_edm_type  # type: ignore[no-redef]
...
class SDKGenerator:
    ...
    def generate_enums(self):
        ...
        for name, enum in self.metadata.enums.items():
            ...
            lines.append("")
            # ``<Enum>Field`` aliases: type checkers see ``Enum | Literal[...]``
            # so plain strings pass mypy, but at runtime pydantic only sees the
            # StrEnum — a duplicate Literal arm roughly doubles every enum
            # field's core schema (~2.4 GiB -> ~1.4 GiB for a full preload).
            if self._has_literal_alias(name):
                members_str = ", ".join(f"'{m.name}'" for m in enum.members)
                lines.append("if TYPE_CHECKING:")
                lines.append(f"    {name}Field = {name} | Literal[{members_str}]")
                lines.append("else:")
                lines.append(f"    {name}Field = {name}")
                lines.append("")
        enums_file.write_text("\n".join(lines))

    def _has_literal_alias(self, enum_name: str) -> bool:
        """Enums that get a ``<Enum>Field`` Literal alias (realistic sizes only)."""
        enum_def = self.metadata.enums.get(enum_name)
        return bool(enum_def and enum_def.members and len(enum_def.members) <= 50)
    ...
    def generate_complex_types(self):
        ...
        for name, ct in self.metadata.complex_types.items():
            ...
            for prop in ct.properties:
                ...
                if py_name in (py_type, "Any", "List", "Optional", "dict", "list", "str", "int", "float", "bool", "none", "Literal"):
                    py_name = f"{py_name}_"

                type_annotation = py_type
                if py_type in self.enum_names and self._has_literal_alias(py_type):
                    # The alias is the bare enum at runtime; type checkers see
                    # Enum | Literal[...] so plain strings still pass mypy.
                    type_annotation = f"{py_type}Field"

                lines.append(f"    {py_name}: {type_annotation} | None = PydanticField(None, alias='{prop.name}')")
            lines.append("")
        ct_file.write_text("\n".join(lines))
        ...
        for domain, entities in domains.items():
            ...
            for entity in entities:
                ...
                for prop in props:
                    ...
                    # Shadowing check (if after conversion it matches a type name)
                    if py_name in (py_type, "Any", "List", "Optional", "dict", "str", "int", "float", "bool", "none", "Literal"):
                        py_name = f"{py_name}_"

                    type_annotation = py_type
                    if py_type in self.enum_names and self._has_literal_alias(py_type):
                        # The alias is the bare enum at runtime; type checkers
                        # see Enum | Literal[...] so plain strings pass mypy.
                        type_annotation = f"{py_type}Field"
    ...
    def generate_fields(self):
        """Generate FieldEnum classes for all entities and complex types to provide autocomplete constants with operator overloading."""
        ...
        for name, ct in sorted(self.metadata.complex_types.items()):
            lines_ct.append(f"class {name}Fields(FieldEnum):")
            if not ct.properties:
                ...
                for prop in ct.properties:
                    ...
                    if py_name in seen_ct:
                        continue
                    seen_ct.add(py_name)
                    lines_ct.append(f"    {py_name} = ODataField('{prop.name}')")
            lines_ct.append("")
        ...
        for domain, entities in domains.items():
            ...
            for entity in sorted(entities, key=lambda x: x.name):
                lines.append(f"class {entity.name}Fields(FieldEnum):")
                ...
                for prop in all_props:
                    ...
                    lines.append(f"    {py_name} = ODataField('{prop.name}')")
                lines.append("")
    ...
    def generate_entities_init(self):
        """Generates src/b1sl/b1sl/models/_generated/entities/__init__.py with lazy rebuild support."""
        ...
        #    Overrides are NOT imported here: this package must never import
        #    hand-written code at init time, or importing an override module
        #    first creates a circular import. They are blended lazily by
        #    _apply_overrides() below.
        ...
        # 3. Lazy per-model rebuild. Building every core schema eagerly costs
        #    ~13s and ~2.4 GiB; building only the models an app touches costs
        #    a few hundred ms and tens of MiB (see todo.md).
        lines.append("# Master namespace for cross-domain resolution")
        lines.append("_NAMESPACE = {m.__name__: m for m in _ALL_MODELS}")
        lines.append("")
        lines.append("_REBUILD_LOCK = threading.RLock()")
        lines.append("_OVERRIDES_APPLIED = False")
        lines.append("")
        lines.append("")
        lines.append("def _apply_overrides():")
        lines.append('    """Blend hand-written ``_overrides`` subclasses into the master namespace.')
        lines.append("")
        lines.append("    Discovery is dynamic: any class defined in a")
        lines.append("    ``b1sl.b1sl.models._overrides.<module>`` that shares its name with a")
        lines.append("    generated entity and subclasses it replaces the generated class —")
        lines.append("    no registration or regeneration required.")
        lines.append("")
        lines.append("    Runs once, lazily, after this package has fully imported: importing")
        lines.append("    overrides at package-init time would create a circular import whenever")
        lines.append("    an override module is itself the first thing imported.")
        lines.append('    """')
        lines.append("    global _OVERRIDES_APPLIED")
        lines.append("    if _OVERRIDES_APPLIED:")
        lines.append("        return")
        lines.append("    with _REBUILD_LOCK:")
        lines.append("        if _OVERRIDES_APPLIED:")
        lines.append("            return")
        ...
        lines.append("")
        lines.append("        from ... import _overrides")
        lines.append("")
        lines.append("        for module_info in pkgutil.iter_modules(_overrides.__path__):")
        lines.append("            module = importlib.import_module(")
        lines.append('                f"{_overrides.__name__}.{module_info.name}"')
        lines.append("            )")
        lines.append("            for attr, obj in list(vars(module).items()):")
        lines.append("                base = _NAMESPACE.get(attr)")
        lines.append("                if (")
        lines.append("                    base is None")
        lines.append("                    or not isinstance(obj, type)")
        lines.append("                    or obj.__module__ != module.__name__")
        lines.append("                ):")
        lines.append("                    continue")
        lines.append("                if not issubclass(obj, base):")
        lines.append("                    # logging, not warnings: b1sl/__init__ installs a broad")
        lines.append("                    # ignore filter for warnings raised from b1sl modules.")
        lines.append("                    logging.getLogger(__name__).warning(")
        lines.append('                        "Override %s.%s shadows entity %r but does not"')
        lines.append('                        " subclass it — ignored.",')
        lines.append("                        module.__name__,")
        lines.append("                        attr,")
        lines.append("                        attr,")
        lines.append("                    )")
        lines.append("                    continue")
        lines.append("                _NAMESPACE[attr] = obj")
        lines.append("                globals()[attr] = obj")
        lines.append("        _OVERRIDES_APPLIED = True")
        lines.append("")
        lines.append("")
        lines.append("def master_namespace():")
        lines.append('    """Override-aware name → model mapping used for forward-ref resolution."""')
        lines.append("    _apply_overrides()")
        lines.append("    return _NAMESPACE")
        lines.append("")
        lines.append("")
        lines.append("def resolve(name):")
        lines.append('    """Return the override-aware, schema-built model class for *name*."""')
        lines.append("    return ensure_built(master_namespace()[name])")
        lines.append("")
        lines.append("")
        lines.append("def ensure_built(model):")
        lines.append('    """Build the model\'s pydantic core schema on first use (thread-safe, idempotent).')
        lines.append("")
        lines.append("    Building eagerly for every entity costs gigabytes of RSS; each model's")
        lines.append("    transitive closure is rebuilt only when first accessed via the facade.")
        lines.append('    """')
        lines.append("    if not model.__pydantic_complete__:")
        lines.append("        namespace = master_namespace()")
        lines.append("        with _REBUILD_LOCK:")
        lines.append("            if not model.__pydantic_complete__:")
        lines.append("                model.model_rebuild(_types_namespace=namespace)")
        lines.append("    return model")
        lines.append("")
        lines.append("")
        lines.append("def preload():")
        lines.append('    """Eagerly build every entity model (the full graph: several GiB, ~10s+).')
        lines.append("")
        lines.append("    Opt-in warm-up for deployments that prefer paying the cost at startup.")
        lines.append('    """')
        lines.append("    for _model in master_namespace().values():")
        lines.append("        ensure_built(_model)")
        lines.append("")
    ...
    def generate_entities_facade(self):
        ...
        lines = [
            ...
            "#",
            "# Entity classes are resolved lazily (PEP 562): the pydantic core schema",
            "# of a model is built on first attribute access, not at import time.",
            "# Building all ~280 entity graphs eagerly costs ~13s and ~2.4 GiB RSS;",
            "# lazy access builds only the models the application actually uses.",
            "# Call entities.preload() to opt into eager warm-up at startup.",
            "",
            "from typing import TYPE_CHECKING, Any",
            "",
        ]
        ...
        # Enums and complex types carry no unresolved forward references, so they
        # are already built at class-definition time — re-export them eagerly.
        ...
        # Static-analysis-only imports: mypy/IDEs resolve en.Item even though the
        # class is loaded lazily at runtime (see __getattr__ below).
        lines.append("if TYPE_CHECKING:")
        ...
        for es_name, es in sorted(self.metadata.entity_sets.items()):
            ...
            if singular != es.entity_type and es.entity_type in all_entities:
                ...
                alias_targets[singular] = es.entity_type
        aliases = list(alias_targets)
        ...
        lines.append("")
        lines.append("# Entity names resolvable through __getattr__ (lazily built on first access)")
        lines.append("_ENTITY_NAMES = frozenset({")
        for name in sorted(all_entities):
            lines.append(f'    "{name}",')
        lines.append("})")
        lines.append("")
        lines.append("# Alias → canonical entity name")
        lines.append("_ALIASES = {")
        for name in sorted(alias_targets):
            lines.append(f'    "{name}": "{alias_targets[name]}",')
        lines.append("}")
        lines.append("")
        lines.append("")
        lines.append("def __getattr__(name: str) -> Any:")
        lines.append('    """Resolve an entity class lazily, building its pydantic schema on first use."""')
        lines.append("    target = _ALIASES.get(name, name)")
        lines.append("    if target not in _ENTITY_NAMES:")
        lines.append("        raise AttributeError(f\"module {__name__!r} has no attribute {name!r}\")")
        lines.append("    from b1sl.b1sl.models._generated import entities as _gen")
        lines.append("")
        lines.append("    model = _gen.resolve(target)  # override-aware + lazily schema-built")
        lines.append("    globals()[name] = model  # cache: subsequent accesses bypass __getattr__")
        lines.append("    return model")
        lines.append("")
        lines.append("")
        lines.append("def __dir__() -> list[str]:")
        lines.append("    return sorted(set(globals()) | _ENTITY_NAMES | set(_ALIASES))")
        lines.append("")
        lines.append("")
        lines.append("def preload() -> None:")
        lines.append('    """Eagerly build every entity model (several GiB of RSS, ~10s+ of CPU).')
        lines.append("")
        lines.append("    Opt-in warm-up for deployments that prefer paying the full schema-build")
        lines.append("    cost once at startup instead of lazily per entity.")
        lines.append('    """')
        lines.append("    from b1sl.b1sl.models._generated import entities as _gen")
        lines.append("")
        lines.append("    _gen.preload()")
