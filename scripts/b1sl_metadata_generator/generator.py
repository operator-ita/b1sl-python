    ...
    def generate_complex_types(self):
        ...
        for name, ct in self.metadata.complex_types.items():
            ...
            for prop in ct.properties:
                ...
                if py_type in self.enum_names:
                    enum_def = self.metadata.enums.get(py_type)
                    if enum_def and enum_def.members:
                        members_str = ", ".join(f"'{m.name}'" for m in enum_def.members)
                        if len(enum_def.members) <= 50: # Only add Literals for realistically sized enums to avoid massive files
                             type_annotation = f"{py_type} | Literal[{members_str}]"
            ...
            for entity in entities:
                ...
                for prop in props:
                    ...
                    if py_type in self.enum_names:
                        ...
                        if enum_def and enum_def.members:
                            members_str = ", ".join(f"'{m.name}'" for m in enum_def.members)
                            if len(enum_def.members) <= 50: # Limit literal size for readability
                                 type_annotation = f"{py_type} | Literal[{members_str}]"
